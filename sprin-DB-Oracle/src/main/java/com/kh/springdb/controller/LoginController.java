...
public class LoginController {
	...
	//  /user/login 경로로 들어오는 POST 요청을 처리해서 실제 로그인을 수행할 수 있도록 진행해줌
	// public String login 메서드는 사용자가 입력한 이름, 사용자가 입력한 이메일을 받아와서
	// LoginService를 통해 실제 로그인을 시도
	public String login(String mname, String memail, Model model) {
		...
		if(userInfo != null) { //만약에 로그인을 성공하게 된다면
			...
			// 		/user/login 페이지로 이동할 수 있도록 지정해 줌
			return "login";
		}
	}
}